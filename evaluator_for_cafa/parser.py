        ...
        for line in f:
            ...
            if line and len(line) > 1:
                ...
                if k == "id":
                    ...
                    if (
                        term_id is not None and obsolete is False and
                    ...
                    k == "relationship" and v.startswith("part_of") and
                    "part_of" in valid_rel
                ):
                    s = v.split()[1].strip()
                    rel.append(s)
...
def gt_parser(gt_file, ontologies):
            ...
            if line:
                ...
                for ont in ontologies:
                    if term_id in ont.terms_dict:
                        gt_dict.setdefault(ont.namespace, {}).setdefault(
                            p_id, []
                        ).append(term_id)
                        break
    ...
    for ont in ontologies:
        if gt_dict.get(ont.namespace):
            matrix = np.zeros(
                (len(gt_dict[ont.namespace]), ont.idxs), dtype='bool'
            )
            ids = {}
            ...
            logging.debug(
                "gt matrix propagated {} {} ".format(ont.namespace, matrix)
            )
            gts[ont.namespace] = GroundTruth(ids, matrix, ont.namespace)
            logging.info(
                'Ground truth: {}, proteins {}'.format(ont.namespace, len(ids))
            )
...
def pred_parser(pred_file, ontologies, gts, prop_mode, max_terms=None):
    """
    Parse a prediction file and returns a list of prediction objects,
    one for each namespace.
    If a predicted is predicted multiple times for the same target, it stores
    the max.
    This is the slow step if the input file is huge, ca. 1 minute for 5GB
    input on SSD disk.
        ...
        for line in f:
            ...
            if line and len(line) > 2:
                ...
                if ns in gts and p_id in gts[ns].ids:
                    i = gts[ns].ids[p_id]
                    if (
                        max_terms is None or
                        np.count_nonzero(matrix[ns][i]) <= max_terms
                    ):
    ...
    for ns in ids:
        if ids[ns]:
            ...
            predictions.append(
                Prediction(ids[ns], matrix[ns], len(ids[ns]), ns)
            )
            logging.info("Prediction: {}, {}, proteins {}".format(
                pred_file, ns, len(ids[ns]))
            )

    if not predictions:
        # raise Exception("Empty prediction, check format")
        logging.warning(
            "Empty prediction! Check format or overlap with ground truth"
        )
...
# 'uniprot:dbReference', namespaces):
