...
def compute_metrics_(tau_arr, g, pred, toi, n_gt):
    ...
    for i, tau in enumerate(tau_arr):
        ...
        # Coverage, number of proteins with at least one term predicted with
        # score >= tau
        ...
        metrics[i, 2] = np.divide(
            n_intersection, n_gt, out=np.zeros_like(n_gt, dtype='float'),
            where=n_gt > 0
        ).sum()
...
def compute_metrics_w_(tau_arr, g, pred, toi, n_gt, ic_arr):
    ...
    metrics = np.zeros((len(tau_arr), 5), dtype='float')
    # cov, wpr, wrc, ru, mi
...
def compute_metrics(pred, gt, tau_arr, toi, toi_ia, ic_arr, n_cpu=0):
    ...
    n_gt = g.sum(axis=1)
    arg_lists = [
        [tau_arr, g, pred, toi, n_gt]
        for tau_arr in np.array_split(tau_arr, n_cpu)
    ]
    with mp.Pool(processes=n_cpu) as pool:
        metrics = np.concatenate(
            pool.starmap(compute_metrics_, arg_lists), axis=0
        )
    columns = ["cov", "pr", "rc"]
    ...
    if ic_arr is not None:
        ...
        arg_lists = [
            [tau_arr, g, pred, toi_ia, n_gt, ic_arr]
            for tau_arr in np.array_split(tau_arr, n_cpu)
        ]
        with mp.Pool(processes=n_cpu) as pool:
            metrics_ia = np.concatenate(
                pool.starmap(compute_metrics_w_, arg_lists), axis=0
            )
...
def evaluate_prediction(
    ...
    for p in prediction:
        ...
        metrics = compute_metrics(
            p, gt[ns], tau_arr, ont.toi, ont.toi_ia, ont.ia, n_cpu
        )

        for column in ["pr", "rc", "wpr", "wrc", "ru", "mi"]:
            if column in metrics.columns:
                if normalization == 'gt' or (
                    column in ["rc", "wrc", "ru", "mi"] and
                    normalization == 'cafa'
                ):
                    # Normalize by gt
        ...
        if ont.ia is not None:
            ...
            metrics['wcov'] = np.divide(
                metrics['wcov'], ne,
                out=np.zeros_like(metrics['wcov'], dtype='float'),
                where=ne > 0)
