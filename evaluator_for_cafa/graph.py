...
class Graph:
    ...
    DAG is the adjacence matrix (sparse) which represent a Directed Acyclic
    Graph where
    ...
    def __init__(self, namespace, terms_dict, ia_dict=None, orphans=False):
        ...
        self.dag = []  # [[], ...] terms (rows, axis 0) x parents (columns,
        # axis 1)
        self.terms_dict = {}  # {term: {index: , name: , namespace: , def: }
        # used to assign term indexes in the gt
        self.terms_list = []  # [{id: term, name:, namespace: , def:, adg: [],
        # children: []}, ...]
        ...
        for self.idxs, (term_id, term) in enumerate(terms_dict.items()):
            rel_list.extend(
                [[term_id, rel, term['namespace']] for rel in term['rel']]
            )
        ...
        for id1, id2, ns in rel_list:
            if self.terms_dict.get(id2):
                ...
                logging.debug(
                    'Skipping branch to external namespace: {}'.format(id2)
                )
        ...
        if orphans:
            self.toi = np.arange(self.dag.shape[0])  # All terms, also those
            # without parents
        else:
            self.toi = np.nonzero(self.dag.sum(axis=1) > 0)[0]  # Only terms
            # with parents
    ...
    def top_sort(self):
        """
        Takes a sparse matrix representing a DAG and returns an array with
        nodes indexes in topological order
        ...
        # for each element of the queue increment visits, add them to the list
        # of ordered nodes
        ...
        # if visited is equal to the number of nodes in the graph then the
        # sorting is complete
        ...
        if visited == rows:
            ...
            raise Exception(
                "The sparse matrix doesn't represent an acyclic graph"
            )
    ...
    def set_ia(self, ia_dict):
        ...
        for term_id in self.terms_dict:
            if ia_dict.get(term_id):
                self.ia[self.terms_dict[term_id]['index']] = ia_dict.get(
                    term_id
                )
            else:
...
class Prediction:
    """
    The score matrix contains the scores given by the predictor for every node
    of the ontology
    """
...
def propagate(matrix, ont, order, mode='max'):
    """
    Update inplace the score matrix (proteins x terms) up to the root taking
    the max between children and parents
    """
    if matrix.shape[0] == 0:
